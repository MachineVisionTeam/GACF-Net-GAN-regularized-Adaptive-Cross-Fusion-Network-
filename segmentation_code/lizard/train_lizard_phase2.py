"""FAN-Seg Phase 2 — Mahmood-inspired adversarial fine-tune on Lizard.

Refactor (v3) from per-instance projection D to whole-image continuous-target D.

Generator G: unchanged from Phase 1 (frozen UNI ViT-L + DPT decoder + HV/Sem3 heads).
Discriminator D: 8-channel input (RGB + HV + Sem3 prob), PatchGAN logits.
  Real pair: image || GT_HV || one_hot(GT_Sem3)
  Fake pair: image || pred_HV || softmax(pred_Sem3)
Both pairs are CONTINUOUS — fixes the binary/soft mismatch that destabilized v1/v2.

Loss (G):  L_sup + lambda_adv * L_G_adv      (lambda_adv = 0.01 default)
Loss (D):  hinge on PatchGAN logits

Usage:
    python train_lizard_phase2.py --split 1 --epochs 30 --batch_size 8 \\
        --lambda_adv 0.01 --warmup_epochs 2
"""
...
def parse_args():
    ...
    p.add_argument("--out_dir", default="/home/sbarua/fanseg/checkpoints/lizard_phase2")
    ...
    p.add_argument("--lambda_adv",  type=float, default=0.01,
                   help="Adversarial weight (Mahmood paper used L1=100 in inverse — small ratio)")
    p.add_argument("--warmup_epochs", type=int, default=2,
                   help="Linear ramp lambda_adv from 0 to target over this many epochs")
    p.add_argument("--depths", type=str, default="5,11,17,23")
    return p.parse_args()


class FanSegLizard(nn.Module):
...
def build_d_input(image_b3hw, hv_b2hw, sem3_b3hw):
    """Concat (RGB, HV, Sem3 prob) along channel -> (B, 8, H, W)."""
    return torch.cat([image_b3hw, hv_b2hw, sem3_b3hw], dim=1)


def sem_index_to_onehot(sem_bhw, num_classes=3):
    """Convert class-index tensor (B, H, W) long -> (B, C, H, W) float one-hot."""
...
def main():
    ...
    if args.phase1_ckpt is None:
        args.phase1_ckpt = f"/home/sbarua/fanseg/checkpoints/lizard_phase1/split{args.split}/best.pt"
    ...
    # === Generator (load from Phase 1) ===
    G = FanSegLizard(depths=depths, decoder_ch=args.decoder_ch).to(device)
    ...
    for ep in range(args.epochs):
        if ep < args.warmup_epochs:
            cur_lambda_adv = args.lambda_adv * (ep + 1) / args.warmup_epochs
        else:
            cur_lambda_adv = args.lambda_adv
        ...
        for batch in train_loader:
            ...
            sem3_prob = torch.softmax(sem3_logits, dim=1)        # (B, 3, H, W) continuous
            sem_onehot = sem_index_to_onehot(sem, num_classes=3) # (B, 3, H, W) continuous

            # ====== Train Discriminator (continuous-vs-continuous) ======
            ...
            # ====== Train Generator (supervised + adversarial) ======
            L_G_total = L_sup
            l_g_adv_val = 0.0
            if cur_lambda_adv > 0:
                fake_x = build_d_input(img, hv_pred, sem3_prob)   # carries grad to G
            ...
            ep_l_sup += float(L_sup.item())
            ep_l_d   += float(L_D.item())
            ep_l_g_adv += l_g_adv_val
            n_batches += 1
        ...
        with torch.inference_mode():
            for batch in val_loader:
                ...
                hv_p, sem3_p = G(img)
                lh = hv_loss(hv_p, hv, fg)
                lm = msge_loss(hv_p, hv, fg)
        ...
        history.append(rec)
        print(f"[ep {ep+1:02d}/{args.epochs}] λ_adv={cur_lambda_adv:.4f} "
