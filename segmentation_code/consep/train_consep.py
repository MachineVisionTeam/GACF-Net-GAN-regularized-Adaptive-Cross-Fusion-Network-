"""Phase 1 (BHF-Net base) trainer on CoNSeP.

Identical to train_lizard.py except loader and output path. Same UNI+DPT+HV/Sem3 stack.
CoNSeP has 4 nuclear classes but Sem3 head outputs 3 channels (bg/fg-interior/boundary),
so class count doesn't matter for Phase 1.

Usage:
    python train_consep.py --split 1 --epochs 60 --batch_size 8
"""
...
def parse_args():
    ...
    p.add_argument("--split", type=int, default=1, help="CoNSeP 5-fold split (1..5)")
    p.add_argument("--out_dir", default="/home/sbarua/fanseg/checkpoints/consep_phase1")
...
def main():
    ...
    model = FanSegCoNSeP(depths=depths, decoder_ch=args.decoder_ch).to(device)
    ...
    for ep in range(args.epochs):
        ...
        with torch.inference_mode():
            for batch in val_loader:
                ...
                v_hv += l_hv.item(); v_ms += l_msge.item(); v_sem += l_sem3.item(); v_n += 1

        rec = {"epoch": ep + 1,
               "train_loss": ep_l/n_b, "train_hv": ep_hv/n_b, "train_msge": ep_ms/n_b, "train_sem3": ep_sem/n_b,
               "val_loss": v_l/max(v_n,1), "val_hv": v_hv/max(v_n,1), "val_msge": v_ms/max(v_n,1), "val_sem3": v_sem/max(v_n,1),
               "lr": sched.get_last_lr()[0], "t_min": (time.time()-t0)/60}
        history.append(rec)
        print(f"[ep {ep+1:02d}/{args.epochs}] train l={rec['train_loss']:.4f} hv={rec['train_hv']:.4f} "
              f"msge={rec['train_msge']:.4f} sem={rec['train_sem3']:.4f} | val l={rec['val_loss']:.4f} "
