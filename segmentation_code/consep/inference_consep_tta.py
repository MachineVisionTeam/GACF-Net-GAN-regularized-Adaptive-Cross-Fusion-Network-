"""Phase 1 TTA inference on CoNSeP (8 augs = 4 rot x 2 flip).
...
class FanSegCoNSeP(torch.nn.Module):
...
def main():
    ...
    if args.ckpt is None:
        args.ckpt = f"/home/sbarua/fanseg/checkpoints/consep_phase1/split{args.split}/best.pt"
    out_dir = f"/home/sbarua/fanseg/predictions/consep/split{args.split}/{args.out_subdir}"
    ...
    model = FanSegCoNSeP(depths=depths, decoder_ch=decoder_ch).to(device)
    model.load_state_dict(cp["model"])
    model.eval()
    print(f"[model] loaded epoch {cp.get('epoch', '?')}")

    ds = CoNSePDataset(split=args.split, mode="test", augment=False)
    ...
    print()
    print("=" * 75)
    print(f"  CoNSeP split {args.split} — TTA (8 augs) results")
    print("=" * 75)
    ...
    out_json = f"/home/sbarua/fanseg/predictions/consep/split{args.split}/_tta_eval_summary.json"
