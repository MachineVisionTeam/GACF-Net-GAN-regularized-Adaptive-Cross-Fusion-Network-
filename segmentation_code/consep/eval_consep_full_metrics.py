"""Full segmentation metrics (ACC, Dice, SE, SP, AJI, PQ, DQ, SQ) on CoNSeP.
Iterates fold{N}_test/labels.npy directly (channel 0 = inst_map) and reads predicted
masks from /home/sbarua/fanseg/predictions/consep/split{N}/{mask_subdir}/.
"""
...
PRED_BASE   = "/home/sbarua/fanseg/predictions/consep"


def eval_split(split, mask_subdir="masks_tta"):
    fold_test_dir = os.path.join(CONSEP_ROOT, f"fold{split}_test")
    gt_labels = np.load(os.path.join(fold_test_dir, "labels.npy"), mmap_mode="r")
    ...
    means["_n_evaluated"] = n_patches - n_missing
    means["_n_missing"] = n_missing
    return means
...
def main():
    ...
    ap.add_argument("--mask_subdir", default="masks_tta",
                    help="masks_tta (Phase 1) or masks_phase2_tta (Phase 2+GAN)")
    ap.add_argument("--out", default=None)
    args = ap.parse_args()
    if args.out is None:
        suffix = "phase2" if "phase2" in args.mask_subdir else "phase1"
        args.out = f"/home/sbarua/fanseg/predictions/consep/_{suffix}_full_metrics_5fold.json"
    ...
    print(f"\n{'='*92}")
    print(f"  5-FOLD MEAN — CoNSeP — {args.mask_subdir}")
    print(f"{'='*92}")
    # AGAFNet paper CoNSeP values (Table III, p. 104)
    paper = {"acc": 98.63, "dice": 85.06, "se": 81.58, "sp": 99.33, "aji": 42.17, "pq": 52.35,
             "dq": None, "sq": None}
    print(f"  {'Metric':<6} {'AGAFNet Paper':>14} {'Ours (5-fold)':>18}   {'Δ vs Paper':>12}")
    print(f"  {'-'*6} {'-'*14} {'-'*18}   {'-'*12}")
    five_fold = {}
