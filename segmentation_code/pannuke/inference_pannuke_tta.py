"""Phase 1 TTA inference on PanNuke (8 augs = 4 rot x 2 flip).
Saves masks and reports PQ/AJI/Dice. Mirrors inference_lizard_tta.py.
"""
...
class FanSegPanNuke(torch.nn.Module):
...
def parse_args():
    ...
    p.add_argument("--out_subdir", default="masks_tta")
...
def main():
    ...
    if args.ckpt is None:
        args.ckpt = f"/home/sbarua/fanseg/checkpoints/pannuke_phase1/split{args.split}/best.pt"
    out_dir = f"/home/sbarua/fanseg/predictions/pannuke/split{args.split}/{args.out_subdir}"
    ...
    model = FanSegPanNuke(depths=depths, decoder_ch=decoder_ch).to(device)
    model.load_state_dict(cp["model"])
    model.eval()
    print(f"[model] loaded epoch {cp.get('epoch', '?')}")

    ds = PanNukeDataset(split=args.split, mode="test", augment=False)
    ...
    print()
    print("=" * 75)
    print(f"  PanNuke split {args.split} — TTA (8 augs) results")
    print("=" * 75)
    ...
    out_json = f"/home/sbarua/fanseg/predictions/pannuke/split{args.split}/_tta_eval_summary.json"
